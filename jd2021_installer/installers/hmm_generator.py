"""
==============================================================================
DEPRECATED / DEAD CODE: HMM PROCEDURAL GENERATOR
==============================================================================

This module was an experimental attempt to generate Kinect gesture `.gesture` 
files from scratch (dynamic generation of the State Table and Edge Table). 

While the binary structure generated by this module was strictly valid according 
to our parser (correct byte sizes, padding, and Zone A/B record sizes), it 
failed in the actual UbiArt game engine with "Error loading resource".

Reason for failure:
The UbiArt engine's Gesture/Classifier system relies on an AdaBoost ensemble 
of decision trees. By procedurally generating random Zone B node types 
(like 3, 8, 9, 10, 18, 19, 20, 28) without the original training data and proper 
decision tree structure, the internal Adaboost graphs became mathematically 
invalid, causing the engine to instantly reject the gesture file (or result 
in 100% misses). 

To solve this, we reverted to a "Hybrid Compilation" approach. The hybrid 
approach bootstraps off of known-good templates (e.g., discorope.gesture), 
retaining its perfectly structured State Table (Zones A & B), and only mutates 
the scoring/gating thresholds (Zone C) to match the new body poses.

This file is preserved for archival and structural reference purposes only.
==============================================================================
