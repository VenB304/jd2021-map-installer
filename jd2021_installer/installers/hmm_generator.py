...
ZONE_B_TYPE_RATIOS = {
    3: 0.20,   # Position (Angle)
    8: 0.15,   # Velocity XYZ
    9: 0.15,   # Velocity magnitude
    10: 0.10,  # Angular Velocity
    18: 0.10,  # Torque X
    19: 0.10,  # Torque Y
    20: 0.10,  # Torque Z
    28: 0.10,  # Speed
...
def _generate_zone_a(
    ...
    final_cycle = cycle  # Track final cycle for Zone B continuity
    ...
    for i in range(zone_a_count):
        ...
        # Correct packing order: [state_id, flag, joint_id, edge_group_type, padding]
        # We restore true edge group mapping to enable full biomechanics logic (velocity, torque, etc).
        # We explicitly set padding to 0, matching official Ubisoft gestures.
        records.extend(struct.pack('<5i', state_id, flag, joint_id, edge_group, 0))
        state_id += 1
        ...
        if states_in_current_cycle >= states_needed:
            group_idx += 1
            states_in_current_cycle = 0
            ...
            if group_idx >= ZONE_A_NUM_EDGE_GROUPS:
                group_idx = 0
                cycle += 1
                final_cycle = cycle  # Update final cycle

        state_to_joint[state_id - 1] = joint_id  # Store the mapping

    return bytes(records), state_to_joint, final_cycle
...
def _generate_zone_b(
    ...
    if zone_b_count <= 0:
        return bytes(records), state_to_joint, cycle
...
def generate_state_table(
    ...
    zone_a_bytes, zone_a_map, zone_a_final_cycle = _generate_zone_a(zone_a_count, jdnext_joint_weights)
    zone_b_bytes, zone_b_map, zone_b_final_cycle = _generate_zone_b(zone_b_count, zone_a_count + 1, zone_a_final_cycle)
...
def build_gesture_binary(
    ...
    # Real Durango format: 23 (sig) + 4 (ver) + 4 (edges) + 4 (states) + 24 (6 fields) = 59
    header = bytearray()
    header.extend(b"GestureDetectorDurango\x00")  # 23 bytes (offset 0-22)
    header.extend(struct.pack('<f', 1.4))           # version  (offset 23-26)
    header.extend(struct.pack('<i', num_edges))     # num_edges (offset 27-30)
    header.extend(struct.pack('<i', num_states))    # num_states (offset 31-34)
    header.extend(struct.pack('<i', 0))             # unknown   (offset 35-38)
    header.extend(struct.pack('<i', 0))             # unknown   (offset 39-42)
    header.extend(struct.pack('<i', 0))             # unknown   (offset 43-46)
    header.extend(struct.pack('<i', 6))             # gesture complexity flag (offset 47-50, mode=6)
    header.extend(struct.pack('<i', 0))             # unknown   (offset 51-54)
    header.extend(struct.pack('<i', 0))             # unknown   (offset 55-58)
